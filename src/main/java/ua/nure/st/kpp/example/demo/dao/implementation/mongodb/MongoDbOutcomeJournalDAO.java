...
public class MongoDbOutcomeJournalDAO implements OutcomeJournalDAO {
    ...
    public Journal readAll() throws DAOException {
        ...
        FindIterable<Document> documents = itemCollection.find().projection(Document.parse("{'income_journal':0}"));
        try (MongoCursor<Document> cursor = documents.cursor()) {
    ...
    public Record read(String id) throws DAOException {
        FindIterable<Document> documents = itemCollection.find(Filters.elemMatch("outcome_journal", eq("_id", new ObjectId(id))))
                .projection(Projections.fields(
                        Projections.elemMatch("outcome_journal", eq("_id", new ObjectId(id))),
                        Projections.include("_id", "vendor", "name", "unit", "weight", "reserve_rate", "amount")));
        try (MongoCursor<Document> cursor = documents.cursor()) {
            if (cursor.hasNext()) {
                Document document = cursor.next();
                Record record = getRecord((Document) document.get("outcome_journal", ArrayList.class).get(0));
                record.setItem(mapToItem(document));
